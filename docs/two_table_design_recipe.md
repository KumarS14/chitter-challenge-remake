...
2. Name of the second table (always plural): `users` 
    ...
    Column names: `email`, `user_name`, `password`
...
1. Can one [user] have many [peeps]? (Yes/No) [YES]
2. Can one [peep] have many [users]? (Yes/No) [NO]
...
```bash
psql -h 127.0.0.1 chitter_database < peeps_and_users_table.sql
```
